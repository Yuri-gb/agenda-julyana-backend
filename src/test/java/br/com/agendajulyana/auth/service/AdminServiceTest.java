...
class AdminServiceTest {
    ...
    void devePermitirAdminCriarOutroAdmin() {
        ...
        when(usuarios.findByEmailIgnoreCase("admin@email.com"))
            .thenReturn(java.util.Optional.of(solicitante));
        when(usuarios.existsByEmailIgnoreCase("novo@email.com")).thenReturn(false);
        when(papeis.findByNome(PapelNome.ADMIN))
            .thenReturn(java.util.Optional.of(papelAdmin));
        when(encoder.encode("senha123")).thenReturn("hash");

        service.criarAdmin(
            "admin@email.com",
            new CriarAdminRequest("Novo Admin", "novo@email.com", "75988887777", "senha123")
        );
    ...
    void deveBloquearClienteDeCriarAdmin() {
        ...
        when(usuarios.findByEmailIgnoreCase("cliente@email.com"))
            .thenReturn(java.util.Optional.of(solicitante));

...
class DisponibilidadeServiceTest {
    ...
    @Test void deveCriarDisponibilidade(){
        when(repository.existeSobreposicao(any(),any(),any(),any())).thenReturn(false);
        when(repository.save(any(Disponibilidade.class))).thenAnswer(i->i.getArgument(0));
        var r=service.criar(new DisponibilidadeRequest(1,LocalTime.of(9,0),LocalTime.of(12,0)), UUID.randomUUID());
    ...
    @Test void deveRejeitarHorarioInvalido(){
        assertThrows(IllegalArgumentException.class,()->service.criar(
            new DisponibilidadeRequest(1,LocalTime.of(12,0),LocalTime.of(9,0)), UUID.randomUUID()));
